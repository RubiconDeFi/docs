...
| Goal  | Endpoint                                      |
| --------------- | -------------------------------------------- |
| GET Market Info for All Markets (24 Hours)   | `https://coingecko.rubicon.finance/tickers` |
| GET Current Order Book Depth for a Pair   | `https://coingecko.rubicon.finance/orderbook?ticker_id=${tickerId}&depth=${depth}` |
| GET Historical Trade Data for a Pair   | `https://coingecko.rubicon.finance/historical_trades?ticker_id=${tickerId}&limit=${limit}&start_time=${startTime}&end_time=${endTime}&type=${type}` |

### Endpoint 1 - /tickers

The `/tickers` endpoint provides 24-hour pricing and volume information on each market pair available and active in this time period on the exchange.
...
The `/orderbook` endpoint provides order book information with an optional depth specified returned for a given market pair/ticker.

#### Request

`GET /orderbook?ticker_id=${tickerId}&depth=${depth}`
...
The `/historical_trades` endpoint returns data on historical completed trades for a given market pair. All input parameters are optional except for `ticker_id`.

#### Request

`GET /historical_trades?ticker_id=${tickerId}&limit=${limit}&start_time=${startTime}&end_time=${endTime}&type=${type}`
...
```sh
curl -X GET "https://coingecko.rubicon.finance/tickers" -H "accept: application/json"
```

2. Get Order Book Depth for a Pair
```sh
curl -X GET "https://coingecko.rubicon.finance/orderbook?ticker_id=0x82af49447d8a07e3bd95bd0d56f35241523fbab1_0xaf88d065e77c8cc2239327c5edb3a432268e5831&depth=100" -H "accept: application/json"
```

3. Get Historical Trade Data for a Pair
```sh
curl -X GET "https://coingecko.rubicon.finance/historical_trades?ticker_id=0x82af49447d8a07e3bd95bd0d56f35241523fbab1_0xaf88d065e77c8cc2239327c5edb3a432268e5831"
```