...
With a valid API key, all RFQ queries should be sent to the base URL of `https://hydrus-serverus-e805ae31d1b4.herokuapp.com` while targetting the relevant endpoint, as described below.
...
```
**Please note, quantity (q) and price (p) are in base amounts represented as a float and directional based on buyToken and sellToken. The user should be aware of the decimals associated with each token to convert these quantity and price values into uint values as needed. Please also note, that qMin must be taken from each order for that leg to be included in your trade - qMin is a mandatory constraint for any given order and represents the minimum quantity required to take for that leg of the trade to be valid.**
