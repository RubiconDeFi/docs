...
```
**Please note, quantity (q) and price (p) are in base uint amounts. The user should be aware of the decimals associated with each token to convert these quantity and price values into human-readable values as needed. Please also note, that qMin must be taken from each order for that leg to be included in your trade.**
